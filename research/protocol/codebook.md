...
- **Window:** each org's first publication → the corpus cutoff (**2026-08-29**; latest publication actually held is 2026-08-27). Later
...
| 9 | Access Type | 📊 | Pre-deployment / Post-deployment / Mixed / N/A. Drives the paper's most policy-relevant split. **`Aggregate` retired 2026-08-31** — it described the report's *format* (a standing scorecard or leaderboard) rather than how the evaluator accessed the model, which is what this column measures. All 85 rows were re-examined against their sources: 76 evaluated released or open-weight models through public/API access → `Post-deployment`; the 9 rows from UK AISI's *Frontier AI Trends Report 2025* → `Mixed`, because the report states its API access to checkpoints "is ahead of public release" in some cases and public in others. |
