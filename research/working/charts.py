#!/usr/bin/env python3
"""Chart suite for AISIEVAL.xlsx — paper-ready: very large type, bright colours, no overlap."""
...
plt.rcParams.update({
 ...
 "axes.edgecolor":"#333333","axes.linewidth":2.2,
 ...
 "axes.grid":True,"grid.color":"#DDDDDD","grid.linewidth":1.6,"axes.axisbelow":True})
...
def hbar(ax,labels,vals,colors,total=None,fs=25):
    ...
    ax.set_xlim(0,m*1.20); ax.grid(axis="y",visible=False)
...
ax.text(.5,.888,"The public-channel AI accountability pipeline",ha="center",fontsize=31,color="#555555")
...
for i,(big,small,col) in enumerate(boxes):
    ...
    ax.text(x+BW/2,.445,small,ha="center",va="center",fontsize=21,color="#333333",linespacing=1.55)
...
ax.text(.5,.035,f"Corpus: {len(ROWS):,} findings · {_nrep} reports · {min(_d)} to {max(_d)}",
        ha="center",fontsize=21,color="#777777")
...
# ---------------------------------------------------------------- 01 institutions
c=collections.Counter(r.get("Institution") for r in ROWS).most_common(18)
fig,ax=plt.subplots(figsize=(19,15))
lab=["\n".join(textwrap.wrap(k,34)) for k,_ in c]
hbar(ax,lab,[v for _,v in c],[BLUE if i%2==0 else SKY for i in range(len(c))],len(ROWS))
ax.set_title(f"Findings per Evaluating Institution (top 18 of "
             f"{len({r.get('Institution') for r in ROWS})})",pad=26)
...
hbar(ax,[k for k,_ in c],[v for _,v in c],[ORANGE if i%2==0 else AMBER for i in range(len(c))],len(A))
...
vbar(ax,[k.replace("-","-\n") if k=="Post-deployment" else k for k in order],v,[BLUE,SKY,PURPLE,TEAL,"#B0B7BF"],len(ROWS))
...
vbar(ax,yrs,[c[y] for y in yrs],[TEAL,BLUE,PURPLE,MAGENTA][:len(yrs)],len(ROWS))
...
ax.set_title(f"Tier Distribution (n={len(ROWS):,})",pad=26); ax.set_ylabel("Findings")
...
d=collections.Counter()
for r in ROWS:
    for p in [x.strip() for x in r.get("Institution Type","").split(";") if x.strip()]: d[p]+=1
c=d.most_common()
fig,ax=plt.subplots(figsize=(18,10))
hbar(ax,["\n".join(textwrap.wrap(k,24)) for k,_ in c],[v for _,v in c],[BLUE,TEAL,PURPLE,MAGENTA,AMBER][:len(c)],len(ROWS))
ax.set_title("Findings per Institution Type",pad=26); ax.set_xlabel("Findings")
save(fig,"13_institution_type.png")
...
ax.bar(range(len(keys)),vals,color=[GREEN,ORANGE,AMBER,PURPLE][:len(keys)],width=0.6,zorder=3)
...
fig,ax=plt.subplots(figsize=(15,10.5))
vbar(ax,["C1\nsignificant risk","C2\nlow risk"],[c["C1"],c["C2"]],[ORANGE,SKY],len(ROWS),fs=29)
...
vbar(ax,["Explicit\nattribution","No explicit\nattribution","No response\nlocated"],
     [c.get(k,0) for k in order],[GREEN,AMBER,"#B0B7BF"],len(A))
