...
- **Window:** each org's first publication → the frozen corpus cutoff (**2026-08-29**). Later
...
LATTICEFLOW · TRANSLUCE · SCALEAI · CIP · PALISADE · DREADNODE · FARAI · RAND · RANDEU · OPENAI ·
ANTHROPIC · AISI-JP (legacy) · CRUX · GRAYSWAN · HOLISTIC · SECUREBIO · SHANGHAIAILAB · UL ·
*(new orgs: add here before first use)*
...
Currently registered: `TRANSLUCE-2026-07-SOC2`. Headline effect at the current corpus:
population 187 → 188, gap 112/187 = 59.9% → 113/188 = 60.1%. (Recomputed 2026-08-31; the figures
first recorded here, 91/147 → 92/148, were correct for the 456-finding corpus of 2026-08-17.)
...
## 9. Column reference (all 39 columns)
...
| 9 | Access Type | 📊 | Pre-deployment / Post-deployment / Mixed / N/A. `Aggregate` retired 2026-08-31 (see codebook col 9). Drives the paper's most policy-relevant split. |
...
| 18 | Attribution | 📊 | Tier A only, **three** categories: **Explicit attribution** (company names the evaluator/finding) / **No explicit attribution** (a response exists but does not name the evaluator) / **No response located** (nothing found). The third is not optional: `Attribution == "No response located"` ⇔ `Action Level == "None"`, and the validator enforces it. Corrected 2026-08-31 — this entry documented two categories while the sheet had always used three. |
...
- **2026-08-29 · Corpus cutoff moved 2026-07-30 → 2026-08-29.** The window is now each organisation's first
  publication → 29 August 2026. This supersedes the 2026-08-03 entry below, which had corrected the header
  back to 2026-07-30, and it resolves the standing discrepancy between §3 (30 July) and the
  workbook's latest publication date (31 July). The extension sweep of 29 August 2026 was run against
  2018-01-01 → 2026-08-29: the back catalogue was re-screened from 2018 (earlier items exist in the
  ledger back to 1962 but no frontier-AI evaluation predates 2018), and 31 July – 29 August 2026
  was enumerated across the 32 active roster venues. Material published after 29 August 2026 is
  `POST-CUTOFF` and belongs to the next version.
...
- **2026-08-31 · `Aggregate` retired from Access Type.** The value described report *format*, not model access, so it could not answer the question the column exists to answer. All 85 rows re-examined against source: 76 → `Post-deployment` (released/open-weight models via public or API access), 9 → `Mixed` (UK AISI Frontier AI Trends Report 2025, whose access is "ahead of public release" in some cases). Pre/post split after recoding: 7.0% vs 80.5% no documented action, z=-8.39, p<0.001.

- **2026-08-29 · window-extension sweep merged.** 167 findings added (Tier A 22 · B 103 · C 42), corpus 1001 -> 1168, Tier A 184 -> 206. Window 2018-01-01..2026-08-29. Prefixes registered on first use: `CRUX` (Princeton CRUX), `RANDEU` (RAND Europe), plus `GRAYSWAN`, `HOLISTIC`, `SECUREBIO`, `SHANGHAIAILAB`, `UL` formalised from existing practice. Findings landing in a report V13 already held inherit that report's Report ID, Title, Date and URL per §6.2 rather than minting a second ID for the same report. 3 items quarantined: 2 university-only authors (no registered evaluator), 1 undatable. Severity and Channel A/B/C pending for the new rows.

- **2026-08-31 · sweep rows completed and the corpus corrected.** The severity ensemble and
  Channels A, B and C have been run on every row added by the sweep; no Tier A row now lacks
  Action Level, Policy Level, severity or proportionality. Three later corrections moved the
  totals off the figures in the entry above: 8 duplicates removed, 103 rows deleted for naming no
  frontier model or system, and 115 findings recovered from `findings_corrected.json`, a directory
  the first merge never read. A further 3 Tier C rows (`UKAISI-2026-08-ALI3`, `-ALI4`, `-GOV1`)
  were deleted on 2026-08-31 under the same frontier rule. Corpus at that point 1,169 · 484 reports ·
  47 institutions · Tier A 231 · B 607 · C 331 · headline 113/188 = 60.1%.
